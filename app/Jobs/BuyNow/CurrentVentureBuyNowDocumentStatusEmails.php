...
class CurrentVentureBuyNowDocumentStatusEmails implements ShouldQueue
{
    ...
    public function handle()
    {
        if(!is_null($this->user) && !is_null($this->type)){
            $email = new \App\Mail\BuyNow\CurrentVentureBuyNowDocumentStatusEmails($this->user, $this->type, $this->document_hash, $this->venture, $this->subject);
            Mail::to(!is_null($this->to) ? $this->to : $this->user->email)->send($email);
        }

        // if($this->send_to_admin == 'true'){
        //     $email = new \App\Mail\BuyNow\CurrentVentureBuyNowDocumentStatusEmailsForAdmin($this->user, $this->type, $this->venture);
